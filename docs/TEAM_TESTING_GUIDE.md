...
>
> Single self-contained document: Parts A–D are the hands-on test plan,
> and **Appendix C** embeds the release acceptance checklist (UAT) so the
> team tests against the same criteria as the Release Manager.
>
...
# Appendix C — Release acceptance checklist (UAT)

> This is the permanent **UAT checklist** from `docs/UAT.md` — the
> canonical release gate run by the Release Manager on a clean machine
> before every release. Included here so the team tests against the same
> checklist; `docs/UAT.md` remains the source of truth.
>
> "Pass" means the documented behavior was observed; any deviation is a
> release blocker unless the Release Manager records an exception.
...
_Companion: `docs/UAT.md` — release acceptance checklist (canonical copy
of Appendix C) · `README.md` — user docs · `SECURITY.md` — responsible
disclosure._
