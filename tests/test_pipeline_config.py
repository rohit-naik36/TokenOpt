...
def test_defaults_enable_all_stages():
    ...
    ctx = _pipeline(config, "router", "compressor", "summarizer").run(
        _messages(), "gpt-4o"
    )
...
class _MutateThenBoomStage(PipelineStage):
    name = "mutate_then_boom"

    def process(self, ctx: OptimizationContext) -> OptimizationContext:
        ctx.messages = [{"role": "user", "content": "corrupted"}]
        ctx.model = "bad-model"
        raise RuntimeError("stage exploded after mutation")


def test_stage_failure_rolls_back_partial_mutation():
    config = TokenOptConfig()
    messages = _messages()

    pipeline = OptimizationPipeline([_MutateThenBoomStage()], config)

    ctx = pipeline.run(messages, "gpt-4o")

    assert ctx.messages == messages
    assert ctx.model == "gpt-4o"
    assert "mutate_then_boom_error" in ctx.metrics


class _NestedMutateThenBoomStage(PipelineStage):
    name = "nested_mutate_then_boom"

    def process(self, ctx: OptimizationContext) -> OptimizationContext:
        ctx.messages[0]["content"]["text"] = "corrupted"
        raise RuntimeError("stage exploded after nested mutation")


def test_stage_failure_rolls_back_nested_message_mutation():
    config = TokenOptConfig()
    messages = [
        {
            "role": "user",
    ...
    assert ctx.messages[0]["content"]["text"] == "original"
    assert "nested_mutate_then_boom_error" in ctx.metrics
