...
def test_loop_bypasses_skip():
    ...
    decision = CompressionDecision(
        target_technique="skip", aggressiveness_ratio=0.0, target_tokens=100, retry_budget=0
    )
...
def test_loop_accepts_first_pass():
    ...
    text = "a" * 20
    decision = CompressionDecision(
        target_technique="heuristic", aggressiveness_ratio=0.5, target_tokens=10, retry_budget=2
    )
...
def test_loop_retries_and_lowers_aggressiveness():
    ...
    text = "a" * 100
    decision = CompressionDecision(
        target_technique="ml_semantic", aggressiveness_ratio=0.8, target_tokens=20, retry_budget=2
    )
...
def test_loop_fails_open_if_budget_exhausted():
    ...
    text = "a" * 100
    decision = CompressionDecision(
        target_technique="heuristic", aggressiveness_ratio=0.5, target_tokens=50, retry_budget=2
    )
...
def test_loop_fails_open_on_executor_crash():
    ...
    decision = CompressionDecision(
        target_technique="ml_semantic", aggressiveness_ratio=0.5, target_tokens=10, retry_budget=2
    )
    result = loop.optimize_segment("hello world", decision)
