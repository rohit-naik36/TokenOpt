...
FILLER_WORDS_REGEX = re.compile(
    r'\b(?:please|kindly|would you|could you|I think|I believe|in my opinion|basically|essentially|fundamentally)\b',
    re.IGNORECASE
)
...
class Analyzer:
    ...
    def analyze(self, text: str, role: str, segment_id: str, is_last_user_query: bool = False, model: str = "gpt-4o") -> CompressionProfile:
        ...
        signals["conversational_fillers"] = min(fillers / max(token_count, 1) * 10, 1.0) # Scale heuristic
        ...
        compressibility = min((signals["whitespace_ratio"] * 0.5) + (signals["conversational_fillers"] * 0.5) + (1.0 - risk_score), 1.0)
