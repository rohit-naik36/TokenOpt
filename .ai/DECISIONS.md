...
| 17 | TokenOpt is released under the **MIT License** (user decision); package metadata: author `rohit-naik36`, full classifiers, project URLs, PEP 639 SPDX `license = "MIT"` (build floor `setuptools>=77`); author name is the GitHub handle — user can personalize before publishing | Standard permissive license; SPDX expression avoids the deprecated license classifier (setuptools 83 errors on the redundant classifier); build-time-only change, no runtime effect | Accepted (M6) |
